// INFO: Phrases for each level
// TODO: Create 15-20 different phrases in each level to minimise repetition.


const firstLevelPhrase = [
    "Testing 1"
]

const secondLevelPhrase = [
    "Apa khabar world!",
    "Cannot lah you",
    "Oi seriously ah",
]

const thirdLevelPhrase = [
    "Chill lah, no need to rush."
]

const fourthLevelPhrase = [
    "Oi bro, let's go to the mamak tonight."
]

const fifthLevelPhrase = [
    "Just now hor, I makan at the mamak, the briyani rice and fried chicken damn nice sia."
]


export { firstLevelPhrase, secondLevelPhrase, thirdLevelPhrase, fourthLevelPhrase, fifthLevelPhrase };